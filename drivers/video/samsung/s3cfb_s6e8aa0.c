...
static void s6e8ax0_read_reg(struct lcd_info *lcd, u8 addr, u8 len)
{
	int ret = 0, i;
	u8 buf[ARRAY_SIZE(SEQ_PANEL_CONDITION_SET)] = {0,};

	ret = s6e8ax0_read(lcd, addr, len, buf, 0);
	if (ret) {
		for (i = 0; i < len; i++) {
			printk("0x%02x, ", buf[i]);
			if (!((i+1) % 5) && i)
...
static int s6e8ax0_probe(struct device *dev)
{
	...
	s6e8ax0_read_reg(lcd, 0xB4, 5);
	s6e8ax0_read_reg(lcd, 0xF8, ARRAY_SIZE(SEQ_PANEL_CONDITION_SET));
