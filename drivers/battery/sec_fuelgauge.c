						...
						const struct i2c_device_id *id)
{
	...
	pr_debug("%s: SEC Fuelgauge Driver Loading\n", __func__);
	...
	if (fuelgauge->pdata->fg_gpio_irq) {
		irq = gpio_to_irq(fuelgauge->pdata->fg_gpio_irq);
		ret = request_threaded_irq(irq, NULL,
				sec_fg_irq_thread, fuelgauge->pdata->fg_irq_attr,
	...
	pr_debug("%s: SEC Fuelgauge Driver Loaded\n", __func__);
	return 0;
				...
				struct i2c_client *client, pm_message_t state)
{
	if (!sec_hal_fg_suspend(client))
		pr_err("%s: Failed to Suspend Fuelgauge\n", __func__);
...
static int sec_fuelgauge_resume(struct i2c_client *client)
{
	...
	if (!sec_hal_fg_resume(client))
		pr_err("%s: Failed to Resume Fuelgauge\n", __func__);
...
static int sec_fuelgauge_shutdown(struct i2c_client *client)
{
	return 0;
}
