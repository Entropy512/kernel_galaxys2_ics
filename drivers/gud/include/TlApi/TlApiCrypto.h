...
typedef enum {
    ...
    TLAPI_ALG_AES_128_CBC_ISO10126      = (AF_CIPHER | AF_CIPHER_AES | 5),      /**< AES (block length 128) with key size 128 in CBC mode, padding according to the ISO10126 scheme. */
    ...
    TLAPI_ALG_AES_128_ECB_ISO10126      = (AF_CIPHER | AF_CIPHER_AES | 0xb),    /**< AES (block length 128) with key size 128 in ECB mode, padding according to the ISO10126 scheme. */
    ...
    TLAPI_ALG_AES_128_CTR_ISO10126      = (AF_CIPHER | AF_CIPHER_AES | 0xe),    /**< AES (block length 128) with key size 128 in CTR mode, padding according to the ISO10126 scheme. */
    TLAPI_ALG_AES_128_CTR_PKCS7         = (AF_CIPHER | AF_CIPHER_AES | 0xf),    /**< AES (block length 128) with key size 128 in CTR mode, padding according to the PKCS#7 scheme. */
    ...
    TLAPI_ALG_AES_256_CBC_ISO10126      = (AF_CIPHER | AF_CIPHER_AES | 0x14),   /**< AES (block length 128) with key size 256 in CBC mode, padding according to the ISO10126 scheme. */
    ...
    TLAPI_ALG_AES_256_ECB_ISO10126      = (AF_CIPHER | AF_CIPHER_AES | 0x1a),   /**< AES (block length 128) with key size 256 in ECB mode, padding according to the ISO10126 scheme. */
    ...
    TLAPI_ALG_AES_256_CTR_ISO10126      = (AF_CIPHER | AF_CIPHER_AES | 0x1d),   /**< AES (block length 128) with key size 256 in CTR mode, padding according to the ISO10126 scheme. */
    TLAPI_ALG_AES_256_CTR_PKCS7         = (AF_CIPHER | AF_CIPHER_AES | 0x1e),   /**< AES (block length 128) with key size 256 in CTR mode, padding according to the PKCS#7 scheme. */
...
typedef enum {
    ...
    /*------- PSS starts here -------*/
    TLAPI_SIG_RSA_SHA256_PSS                = (AF_SIG | AF_SIG_PSS | 1),   /**< RSASSA-PSS-VERIFY, ContenDigest-SHA256, MfgDigest-SHA256. */
    TLAPI_SIG_RSA_SHA1_PSS                  = (AF_SIG | AF_SIG_PSS | 2)    /**< RSASSA-PSS-VERIFY, ContenDigest-SHA1, MfgDigest-SHA1. */
} tlApiSigAlg_t;
...
typedef enum {
    TLAPI_RSA       = 0x00000001,   /**< RSA public and RSA private key. */
    TLAPI_RSACRT    = 0x00000002    /**< RSA public and RSA CRT private key. */
} tlApiKeyPairType_t;
 ...
 */
typedef struct {
    uint8_t *exponent;      /**< Pointer to exponent (byte array in big endian format). */
    uint32_t exponentLen;   /**< Byte length of exponent. */
    uint8_t *modulus;       /**< Pointer to modulus (byte array in big endian format). */
    uint32_t modulusLen;    /**< Byte length of modulus. */
...
typedef struct {
    tlApiRsaKey_t *privateKey;  /**< Pointer to the private key structure of RSA key pair. */
    tlApiRsaKey_t *publicKey;   /**< Pointer to the public key structure of RSA key pair. */
} tlApiRsaKeyPair_t;
 ...
 */
typedef union {
    tlApiRsaKeyPair_t *rsaKeyPair; /**< Pointer to RSA key pair structure. */
} tlApiKeyPair_t;
 ...
 * The key components are generated according to requested type and length. For RSA the passed
 * length value identifies the length of the modulus, the generated exponent can be of shorter
 * length.
 * The caller has to set addresses in the key pair structure. Generated key components are written
 * to those addresses. It is in the responsibility of the caller to provide sufficient space and
 * set length parameters of each of the buffer length elements in key structures appropriately.
 ...
 * @returns E_TLAPI_CR_WRONG_OUPUT_SIZE     if provided buffer length of one of the buffers is too small.
...
/** Generates a key pair.
 * Same as tlApiGenerateKeyPair() except that all generated components are written into the passed
 * buffer, starting at index 0. No empty bytes between components. Caller does not have to set the
 * pointers in the key pair structure, they are set by the function.
 * It is in the responsibility of the caller to provide sufficient space.
 *
 * @param [in,out]  keyPair     Reference to key pair structure.
 * @param           type        See enum keyPairType_t.
 * @param           len         Requested byte length of keys.
 * @param [out]     buffer      Reference to buffer into which generated key components are written.
 * @param           bufferLen   Length of buffer for generated components.
 *
 * @returns TLAPI_OK                        if operation was successful.
 * @returns E_TLAPI_NULL_POINTER            if one parameter is a null pointer.
 * @returns E_TLAPI_INVALID_RANGE           if buffer is not within the drivers range.
 * @returns E_TLAPI_CR_ALGORITHM_NOT_AVAILABLE if combination of algorithm and mode and key length is not available.
 * @returns E_TLAPI_CR_WRONG_OUPUT_SIZE     if bufferLen is too small.
 * @returns E_TLAPI_DRV_UNKNOWN             if some unknown error occurred.
 */
 ...
 * @returns E_TLAPI_CR_WRONG_OUPUT_SIZE if [in]destLen is inconsistent with algorithm requirements.
 * @returns E_TLAPI_DRV_UNKNOWN         if some unknown error occurred.
 * @returns E_TLAPI_CR_INCONSISTENT_DATA    if key type doesn't match the algorithm.
 */
 ...
 *       ignored and don't result in an error.
 *
 * @note For PSS signatures bufferLen is interpreted as salt length. For verification, the buffer is ignored
 *       and may be NULL, as the salt is already provided within the signature.
 *       For sign operations, the buffer must point to the salt if saltLen > 0.
 *
